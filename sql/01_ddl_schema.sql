...
CONSTRAINT fk_la_isbn FOREIGN KEY (isbn) REFERENCES LIBRO (isbn) ON DELETE CASCADE ON UPDATE CASCADE, 
CONSTRAINT fk_la_autor FOREIGN KEY (id_autor) REFERENCES AUTOR (id_autor) ON DELETE CASCADE ON UPDATE CASCADE 
); 
...
CONSTRAINT fk_lg_genero FOREIGN KEY (id_genero) REFERENCES GENERO (id_genero) ON DELETE CASCADE ON UPDATE CASCADE 
);
...
CONSTRAINT fk_ejemplar_isbn FOREIGN KEY (isbn) REFERENCES LIBRO (isbn) ON DELETE RESTRICT ON UPDATE CASCADE, 
CONSTRAINT chk_estado_fisico CHECK (estado_fisico IN ('BUENO', 'DETERIORADO', 'BAJA')) 
);
...
CONSTRAINT chk_venc_after_prest CHECK (fecha_vencimiento > fecha_prestamo), 
CONSTRAINT chk_devol_after_prest CHECK (fecha_devolucion IS NULL OR fecha_devolucion >= fecha_prestamo) );
...
CONSTRAINT chk_sancion_tipo CHECK (tipo IN ('MORA', 'DAÑO', 'PERDIDA', 'OTRO')), 
